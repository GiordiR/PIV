function image_out = preproc (image_in,order,clahe,clahe_window,hpf,hpf_size,minmax,minmax_size,int_capt,capt_scaling)
    ...
    %
    % Image pre-processing for correlation analysis
    %
    %
    % arguments (input):
    %   image_in - Input image
    %
    %   order - Filter execution order
    %
    %   clahe - Contrast Limited Adaptive Histogram Equalization 
    %           (0=deactivated, 1=activated)
    %
    %   clahe_window - CLAHE window size
    %     
    ...
    %   hpf_size - HPF size
    %  
    %   minmax - Min-Max Filter
    %            (0=deactivated, 1=activated)
    % 
    %   minmax_size - Min-Max FIlter size
    %  
    %   int_capt - Intensity Capping
    %              (0=deactivated, 1=activated)
    %
    %   capt_scaling - Intensity Capping Scaling factor
    %  
    % arguments (output):
                    ...
                    for i=1+neighb:y-neighb
                        for j=1+neighb:x-neighb
                            if rem(minmax_size,2)==0
                                disp('Error: minmax_size has to be an odd number');
                                break
                            else
