    ...
    if method.on_uv == 1
        switch method.choice
            case 0
                ...
                % Verifica compatibilità con i limiti 
                u(u<umin) = NaN;
                u(u>umax) = NaN;
                v(v<vmin) = NaN;
                v(v>vmax) = NaN;
                ...
                if parameter.builtin==0
                    for i=neighb+1:size(u,1)-neighb
                        for j=neighb+1:size(u,2)-neighb
                            ...
                            % Velocità di riferimento per la cella
        ...
        % MISURA DI ACCURATEZZA (PERCENTUALE DI OUTLIERS)
        ...
        fprintf('Percentuale di outliers di u: %f %% \n',error_u);
        fprintf('Percentuale di outliers di v: %f %% \n',error_v);   
        
        % INTERPOLAZIONE DELLE VELOCITA' MANCANTI
        if interp.uv==1
